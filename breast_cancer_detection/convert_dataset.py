...
parser.add_argument('--low-high-mode', type=str, choices=['y', 'n'], default=None,
                    help="Use 'mass_low'/'mass_high' classes. If omitted, asks interactively.")
parser.add_argument('-y', '--yes', action='store_true',
                    help="Auto-confirm any interactive prompts (e.g. removing images/ in mask mode).")
...
# Ask user for low_high_mode unless passed via CLI
if args.low_high_mode is not None:
    low_high_mode = (args.low_high_mode == 'y')
else:
...
def normalize(pixel_array):
    ...
    pixel_range = pixel_array.max() - pixel_min
    if pixel_range == 0:
        return np.zeros_like(pixel_array, dtype=np.float64)
    return (pixel_array - pixel_min) / pixel_range
...
if 'inbreast' in chosen_datasets:
    ...
    for filename, score in zip(inbreast_csv_data['File Name'], inbreast_csv_data['Bi-Rads']):
        ...
        # Some rows may be NaN or have non-digit chars like "4a"
        score_str = re.sub(r'\D', '', str(score))
        if score_str == '':
            continue
        score = int(score_str)  # 4a, 4b, 4c => 4
        file_score_pairs[filename] = score

    # Contadores para log/diagnóstico
    inb_processed = 0
    inb_skipped_no_mask = 0
    inb_skipped_no_roi = 0

    for inbreast_xml in tqdm(inbreast_xmls, desc="Processing INBreast dataset"):
        image_id += 1
        dcm_prefix = None  # aseguramos existencia para casos sin ROI
        txt_lines = []     # reiniciar por imagen
        ...
        for entry in entries:
            ...
            if class_name in chosen_classes:
                # Literal eval to convert string of list to actual list
        ...
        if any(rois.values()):
            ...
            for filename in os.listdir(inbreast_dcm_dir):
                if re.match(dcm_prefix + r'.*\.dcm', filename):
                    ...
                    break
            if not dcm_path:
                print(f"[INBreast][skip] DICOM not found for prefix '{dcm_prefix}'.", file=sys.stderr)
                inb_skipped_no_roi += 1
                continue
            ...
            # Read pixels from DICOM, convert to 0-255 range for JPEG
            ...
            if output_choice in ['coco', 'yolo']:
                ...
                for cls in rois.keys():
                    ...
                    if low_high_mode == True and cls == 'mass':
                        if dcm_prefix in file_score_pairs:
                            score = file_score_pairs[dcm_prefix]
                            if score <= 3:
                                cls_suffix_local = '_low'
                            elif score > 3:
                                cls_suffix_local = '_high'
                    class_id = all_classes.index(cls + cls_suffix_local)
                    # Go through each ROI in the image
                    for roi in rois[cls]:
                        if output_choice == 'coco':
                            # COCO mode
                            roi = roi.astype(int)
                            ...
                            if any([x > 1 for x in bbox]):
                                raise Exception('Bbox calculated relative coord larger than 1 for ROI: ' + str(roi))
        ...
        # Escritura de etiquetas / control de omisiones (solo para YOLO/COCO)
        if output_choice != 'mask':
            # Si no hubo ROIs en el XML
            ...
            if len(txt_lines) > 0:
                ...
                # Antes se hacía: raise Exception("Image without mask")
                print(f"[INBreast][skip] {dcm_prefix}: no se generaron etiquetas (sin máscara/ROI válida).", file=sys.stderr)
                inb_skipped_no_mask += 1

    # Resumen INBreast
    total_inb = inb_processed + inb_skipped_no_mask + inb_skipped_no_roi
    print(f"[INBreast] Procesadas: {inb_processed} | Omitidas sin máscara: {inb_skipped_no_mask} | Omitidas sin ROI: {inb_skipped_no_roi} | Total vistas: {total_inb}")
    ...
    if output_choice == 'yolo':
        for image_path in tqdm(image_mask_pairs.keys(), desc="Processing CBIS-DDSM images"):
            ...
            for mask_path in image_mask_pairs[image_path]:
                ...
                if mask is None:
                    # Archivo de máscara ausente o corrupto
                    print(f"[CBIS-DDSM][skip] máscara no legible: {mask_path}", file=sys.stderr)
                    continue
...
if 'mias' in chosen_datasets:
    ...
    with open(mias_info) as f:  # Reopen file for actual processing
        for line in tqdm(relevant_lines, desc="Processing MIAS dataset"):
            ...
            if image is None:
                print(f"[MIAS][skip] imagen no legible: {image_name}.pgm", file=sys.stderr)
                continue
            ...
            for class_name in chosen_classes:
                if line_parts[2] in mias_chosen[class_name]:
                    ...
                    if any([x > 1 for x in bbox]):
                        raise Exception('Bbox calculated relative coord larger than 1 for ROI: ' + str(bbox))
                    # Txt line
...
if split_mode:
    ...
    if output_choice == 'yolo':
        for directory in split_dirs:
            yolo_to_coco(os.path.join(directory, txt_out_dir), os.path.join(directory, image_out_dir),
                         directory + '.json')
        # Required for YOLO
        yaml_data = {
            'path': os.getcwd(),
