...
def parse_args():
    ...
    parser.add_argument('--n_times', type=int, default=3,
                        help='How many augmented copies to generate per image.')
    parser.add_argument('--seed', type=int, default=42,
                        help='Base random seed for reproducible augmentations.')
    return parser.parse_args()

def augment_worker(args_tuple):
    image_info, image_id_to_annotations, args = args_tuple
    # Cada worker parte de un proceso "fork" con el mismo estado de RNG del padre;
    # seedear por imagen evita augmentations repetidas/no reproducibles entre workers.
    worker_seed = args.seed + image_info['id']
