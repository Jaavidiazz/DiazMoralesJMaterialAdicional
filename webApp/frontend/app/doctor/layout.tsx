...
const doctorLinks = [
  { href: "/doctor/dashboard", label: "Resumen", icon: LayoutDashboard },
  { href: "/doctor/cases/newCase", label: "Nuevo caso", icon: FilePlus2 },
  { href: "/doctor/cases", label: "Casos", icon: FolderOpen },
  { href: "/doctor/informes", label: "Informes", icon: FileText },
];
...
export default function DoctorLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const newInitials = getInitials(fullName, user.email, "DR");
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50">
      ...
      >
        <div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-teal-500/10 blur-3xl" />
        ...
        <div className="relative flex items-center justify-between gap-3 px-6 pt-7 pb-5">
          <div className="flex items-center gap-3">
            <div className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-teal-500 text-white shadow-lg shadow-teal-500/30">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Panel del doctor</p>
            </div>
          </div>
        ...
        <div className="relative px-4 py-4">
          <div className="mb-3 flex items-center gap-2.5 rounded-xl bg-white/5 px-3 py-2.5">
            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-teal-500/20 text-xs font-semibold text-teal-300">
              {initials}
            </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex flex-shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 md:hidden">
          ...
          <div className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-teal-500 text-white">
